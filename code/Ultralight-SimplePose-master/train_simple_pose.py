...
def data_loader(PATH, BATCH_SIZE, IMG_W, IMG_H, HEATMAP_W, HEATMAP_H, num_workers):
    train_dataset = mscoco.keypoints.COCOKeyPoints(PATH, splits='person_keypoints_train2017')
    val_dataset = mscoco.keypoints.COCOKeyPoints(PATH, splits='person_keypoints_val2017')
...
def train():
    # 数据集文件夹路径
    # PATH = 'coco'
    # PATH = 'F:/My_Project/Big_Datasets/coco'
    PATH = 'datasets'
    ...
    # 指定训练的GPU
    num_workers = 1   # 值越大数据读取越快但内存占用越多
    context = mx.gpu(0)
    ...
    for epoch in range(EPOCH):
        ...
        net.export('model/output1/Ultralight-Nano-SimplePose_%.5f' % val_acc, epoch=epoch)
