...
class SimpleHRNet:
    ...
    def _predict_single(self, image):
        if not self.multiperson:
            ...
            detections = self.detector.predict_single(image)        # 这里 detector 就是 yolov3
            ...
            if detections is not None:
                for i, (x1, y1, x2, y2, conf, cls_conf, cls_pred) in enumerate(detections):
                    ...
                    boxes[i] = [x1, y1, x2, y2]         # 标注框
                    images[i] = self.transform(image[y1:y2, x1:x2, ::-1])       # 将 bbox 框里的图像提取出来以进行关键点检测
        ...
        if images.shape[0] > 0:
            ...
            with torch.no_grad():
                if len(images) <= self.max_batch_size:
                    out = self.model(images)            # 这里的 model 就是姿态识别模型
            ...
            for i, human in enumerate(out):
                ...
                for j, joint in enumerate(human):
                    ...
                    # 2: confidences
                    pts[i, j, 0] = pt[0] * 1. / (self.resolution[0] // 4) * (boxes[i][3] - boxes[i][1]) + boxes[i][1]       # 关键点 y
                    pts[i, j, 1] = pt[1] * 1. / (self.resolution[1] // 4) * (boxes[i][2] - boxes[i][0]) + boxes[i][0]       # 关键点 x
                    pts[i, j, 2] = joint[pt]      # 某种比例
        ...
        if self.return_bounding_boxes:
            res.append(boxes)               # 输出边界框
        res.append(pts)                     # 输出关键点
