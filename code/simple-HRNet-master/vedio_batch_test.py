...
'''
    修改后的代码：
        可以同时输出每一帧图片。每一帧图片的关键点可视化图，关键点数据
'''
...
def main(camera_id, filename, hrnet_m, hrnet_c, hrnet_j, hrnet_weights, hrnet_joints_set, image_resolution,
         ...
         video_framerate, device, csv_output_filename, csv_delimiter):     #
    ...
    # 图片路径
    image_path = 'output/image_test/2.jpg'

    # 根目录（每次测试输出结果的路径）
    root = './output/image_test'
    if not os.path.exists(root):
        os.makedirs(root)

    # 保存 frame 帧的路径
    # saveFrame_path = './output/output3/frame_output'
    saveFrame_path = os.path.join(root, 'frame_output')
    if not os.path.exists(saveFrame_path):
        os.makedirs(saveFrame_path)

    # 保存可视化效果图
    # saveView_path = './output/output3/view_output'
    saveView_path = os.path.join(root, 'view_output')
    if not os.path.exists(saveView_path):
        os.makedirs(saveView_path)

    # 关键点文件保存路径
    csv_output_filename = os.path.join(root, args.csv_output_filename)  # args.csv_output_filename 为 output.csv
    ...
    with open(csv_output_filename, 'wt', newline='') as fd:     #
        ...
        model = SimpleHRNet(
            ...
            return_bounding_boxes=not disable_tracking,           # 这个会返回 BBOX 边界框
            # return_bounding_boxes=True,                         # 修改，True 是返回 BBOX 边界框
        ...
        if image_test:
            ...
            # csv format is:
            # 第一行是输出的 BBOX 框，第二行是关键点坐标和相应关键点的置信度
            ...
            for j, pt in enumerate(out):
                row = [0, j] + pt.flatten().tolist()  # index 代表第几帧，j 代表检测到的人
                # print(len(pt.flatten().tolist()))
            ...
            for i, (pt, pid) in enumerate(zip(pts, person_ids)):
                out_image = draw_points_and_skeleton(img, pt, joints_dict()[hrnet_joints_set]['skeleton'], person_index=pid,
                                                 points_color_palette='gist_rainbow', skeleton_color_palette='jet',
                                                 points_palette_samples=10)     # 添加关键点和骨干
            cv2.imshow("Image", out_image)
            cv2.waitKey(0)
            cv2.imwrite('output/image_test/out.jpg', out_image)
        ...
        if video_test:
            ...
            while True:
                ...
                if filename is not None or disable_vidgear:
                    ...
                    '''第一个参数ret 为True 或者 False,代表有没有读取到图片；第二个参数 frame 表示截取到一帧的图片'''
                    ret, frame = video.read()  # 读一帧
                    if not ret:
                        break
                ...
                # 保存每帧图像
                path = saveFrame_path + '/' + str(count) + '.jpg'
                ...
                for j, pt in enumerate(pts):
                    row = [index, j] + pt.flatten().tolist()  # index 代表第几帧，j 代表检测到的人
                    # print(len(pt.flatten().tolist()))
                    csv_output.writerow(row)
                ...
                for i, (pt, pid) in enumerate(zip(pts, person_ids)):
                    frame = draw_points_and_skeleton(frame, pt, joints_dict()[hrnet_joints_set]['skeleton'],
                                                     person_index=pid,
                                                     points_color_palette='gist_rainbow', skeleton_color_palette='jet',
                                                     points_palette_samples=10)  # 添加关键点和骨干

                # 保存得到的每帧关键点图像
                path = saveView_path + '/' + str(count) + '.jpg'
                cv2.imwrite(path, frame)
                count += 1
                ...
                if save_video:
                    if video_writer is None:
                        ...
                        video_writer = cv2.VideoWriter('output.avi', fourcc, video_framerate,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--disable_tracking",
                        help="disable the skeleton tracking and temporal smoothing functionality",  # 禁用框架跟踪和时间平滑功能
                        action="store_true")
    ...
    '''
    parser.add_argument("--image_test", help="test images", type=bool, default=True)
    parser.add_argument("--video_test", help="test video", type=bool, default=False)
    '''
